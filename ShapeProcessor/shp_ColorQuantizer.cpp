...
//---------------------------------------------------------------------------
shp_ColorQuantizer::shp_ColorQuantizer()
{
	m_nColorsCount = 0;
	m_pImage = new shp_Image();
}
//---------------------------------------------------------------------------
shp_ColorQuantizer::~shp_ColorQuantizer()
{
...
//---------------------------------------------------------------------------
void shp_ColorQuantizer::Assign(shp_Image *pImage)
{
	m_pImage->Assign(pImage);
    LogPal SysPal;
    FillMemory(SysPal.lpal.palPalEntry,256*4,NULL);
    m_nColorsCount = ::GetPaletteEntries(m_pImage->pBitmap->Palette,0,255,SysPal.lpal.palPalEntry);
    int K;
	for(int k=0;k<m_nColorsCount;k++)
    {
        K=3*k;

        pPalette[K+2] = SysPal.lpal.palPalEntry[k].peRed;
        pPalette[K+1] = SysPal.lpal.palPalEntry[k].peGreen;
        pPalette[K+0] = SysPal.lpal.palPalEntry[k].peBlue;

        pColors[k]=(TColor)RGB(pPalette[K+2],
                                   pPalette[K+1],
                                   pPalette[K+0]);
    }
}
...
#define WEIG // >>1 // >>2
//---------------------------------------------------------------------------
void shp_ColorQuantizer::Quantize(UINT nColorsCount,bool bCreatePalette,bool bFullPalette)
{
    ...
    TrueColorPic = m_pImage;

    ImageSize = m_pImage->Width*m_pImage->Height;
    if(nColorsCount > 256)	nColorsCount = 256;
    PalSize = nColorsCount;
    m_nColorsCount = 0;
    ...
    if(bCreatePalette)
    {
        ...
        if(bFullPalette)
	        ...
	        SysPal.lpal.palNumEntries = m_nColorsCount;

        int K;
        for (k = 0; k < m_nColorsCount; ++k) {
    		Mark(&cube[k], k, tag);
    		...
    		if (weight)
            {
    			pPalette[K+2] = (Byte)(Vol(&cube[k], mr) / weight) WEIG;
    			pPalette[K+1] = (Byte)(Vol(&cube[k], mg) / weight) WEIG;
    			pPalette[K+0] = (Byte)(Vol(&cube[k], mb) / weight) WEIG;
    		}
            else
            {
    			pPalette[K+2] =
            ...
            SysPal.lpal.palPalEntry[k].peRed=pPalette[K+2];
            SysPal.lpal.palPalEntry[k].peGreen=pPalette[K+1];
            SysPal.lpal.palPalEntry[k].peBlue=pPalette[K+0];
            SysPal.lpal.palPalEntry[k].peFlags=PC_RESERVED;
        }

        if(bFullPalette)
        {
	        for(int k=m_nColorsCount;k<nFull;k++)
	        {
        ...
        m_pImage->pBitmap->Palette=::CreatePalette(&SysPal.lpal);
        m_pImage->UpdateLines();

        i=0;
        for(UINT y=0;y<m_pImage->Height;y++)
        {
            for(UINT x=0;x<m_pImage->Width;x++)
            {
                m_pImage->ppLines[y][x]=tag[Qadd[i]];
                i++;
            }
        }
    }
    else
    {
        ...
        for(UINT y=0;y<m_pImage->Height;y++)
        {
            for(UINT x=0;x<m_pImage->LineSize;x+=3)
            {
                K=3*tag[Qadd[i]];
                m_pImage->ppLines[y][x+2]=pPalette[K+2];
                m_pImage->ppLines[y][x+1]=pPalette[K+1];
                m_pImage->ppLines[y][x  ]=pPalette[K];
                i++;
            }
        }
    }
