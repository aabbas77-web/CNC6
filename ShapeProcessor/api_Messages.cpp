...
int CALLBACK DwgProc (int iDwg,UINT Msg,int Prm1,int Prm2,double Prm3,double Prm4,void* Prm5)
{
    switch( Msg )
    {
        ...
        case VM_ENTADD:
        {
        	int nEntityType = Prm1;
            int nEntityIndex = Prm2;
            switch(nEntityType)
            {
             	case VL_ENT_POLY:
                {
                	FormMain->Contour.clear();
            		int nCount = vlPropGetInt(VD_POLY_N_VER,nEntityIndex);
                    double x,y,parm;
                    for(int i=0;i<nCount;i++)
                    {
                    	vlPolyVerGet(nEntityIndex,i,&x,&y,&parm);
                        FormMain->Contour.push_back(shpPoint(x,y));
                    }
                 	break;
                }
            }
